...
<%
	...
	//2. DB 에서 해당 파일의 정보를 읽어온다. // 번호를 이용해서 saveFileName 을 읽어온다.
	FileDto dto=FileDao.getInstance().getData(num);
	//로그인된 아이디와 글 작성자가 같을때만 삭제 되도록한다.
	String id=(String)session.getAttribute("id");
	if(!dto.getWriter().equals(id)){
		//.sendError(에러코드, 에러메세지);
		response.sendError(HttpServletResponse.SC_FORBIDDEN, "남의 파일을 지우지 마세요");
		return;//메소드 끝내기 (service()메소드)
	};
	//3. 파일 시스템에서 해당 파일을 삭제한다. //파일 객체의 delete 기능을 써서 삭제
