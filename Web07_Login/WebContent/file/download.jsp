...
<%
	...
	//3. 해당 파일을 실제로 다운로드 시켜준다.(응답)  //다운로드 하기 위해서 원본 파일명, 저장된 파일명, 파일사이즈가 필요하다.
	//다운로드 시켜주기 ... 
	String orgFileName=dto.getOrgFileName(); //원본 파일명
	String saveFileName=dto.getSaveFileName(); //저장된 파일명 // 업로드 파일을 어떤 이름으로 저장을했는지 때문에 필요
	
	//다운로드 시켜줄 파일의 실제 경로 구성하기 // File.separator 은 영역구분 윈도우는 \ 리눅스에서는 /
	String path=application.getRealPath("/upload")
			+File.separator+saveFileName; //이 경로가 어디에 잡힐지 써주는것 webcontent/upload
